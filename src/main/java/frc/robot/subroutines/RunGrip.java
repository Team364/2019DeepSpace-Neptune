...
public class RunGrip extends CommandGroup {
  ...
  public RunGrip(int set) {
    ...
    addSequential(new SetPiston(Robot.trident.lever, 1)); //Open lever
    System.out.println("Grabbing Hatch");
    }else if(set == 3){ //Score Cargo
    addSequential(new runIntake(0.75, false)); //Outtake
    addSequential(new SetPiston(Robot.trident.claw, 1)); //Open Claw
    System.out.println("Scoring Cargo");
    }else if(set == 4){  //Score Hatch
    addSequential(new SetPiston(Robot.trident.lever, 0)); //Close lever
    addSequential(new WaitCommand(0.2));
    addSequential(new SetPiston(Robot.trident.claw, 1));
